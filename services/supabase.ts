...
export const SupabaseService = {
  ...
  async getProfiles() {
    try {
      ...
      console.warn("Aviso: Falha ao ler perfis (RLS pode estar ativo):", err.message);
  ...
  async getEmpreendimentos() {
    try {
      const { data, error } = await supabase.from('empreendimentos').select('*').order('created_at', { ascending: false });
      if (error) throw error;
      return { 
        data: (data || []).map(item => ({
          id: item.id,
